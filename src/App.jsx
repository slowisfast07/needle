...
function Carousel({ th, children, speed = 0.5 }) {
  ...
  useEffect(() => {
    ...
    const step = () => {
      if (el && !pausedRef.current) {
        const loop = (groupRef.current?.offsetWidth || 0) + 12; // one group + gap
        posRef.current += speed;
        if (loop > 0 && posRef.current >= loop) posRef.current -= loop;
        el.scrollLeft = posRef.current; // set from float accumulator (Safari-safe)
      }
  ...
  const sync = () => { if (ref.current) posRef.current = ref.current.scrollLeft; };
  const pause = () => { pausedRef.current = true; };
  const resume = () => { sync(); pausedRef.current = false; };
  const nudge = (dir) => { const el = ref.current; if (el) { el.scrollBy({ left: dir * 300, behavior: "smooth" }); setTimeout(sync, 360); } };
  const arrow = (dir) => ({
  ...
  return (
    <div style={{ position: "relative" }}
      ...
      <div ref={ref} className="cax" style={{ display: "flex", gap: 12, overflowX: "auto", padding: "2px 40px", scrollbarWidth: "none" }}>
        <div ref={groupRef} style={{ display: "flex", gap: 12, flexShrink: 0 }}>{children}</div>
